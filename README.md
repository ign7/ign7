<h1 align="center">👨‍💻 Igor Frederico Gomes Quaresma</h1>
<h3 align="center">🚀 Full Stack Laravel Developer</h3>

<p align="center">
  Software Developer focused on scalable systems, ERP platforms, APIs, dashboards and business solutions.
</p>

<p align="center">
  🎓 Information Systems — PUC Minas <br>
  🌎 Constant Learner <br>
  ⚡ Laravel Ecosystem Enthusiast
</p>

<hr>

<h2 align="center">⚡ About Me</h2>

<table align="center">
  <tr>
    <td width="50%" valign="top">
      <h3>💻 Backend</h3>
      <ul>
        <li>Laravel Applications</li>
        <li>REST APIs</li>
        <li>ERP Systems</li>
        <li>Financial Systems</li>
        <li>Business Rules Automation</li>
        <li>PHP & MySQL</li>
      </ul>
    </td>
    <td width="50%" valign="top">
      <h3>🎨 Frontend</h3>
      <ul>
        <li>Vue.js Interfaces</li>
        <li>Livewire Components</li>
        <li>TailwindCSS</li>
        <li>Responsive UI</li>
        <li>JavaScript & jQuery</li>
        <li>Dashboard Development</li>
      </ul>
    </td>
  </tr>
</table>

<hr>

<h2 align="center">🛠️ Tech Stack</h2>

<h3>🔙 Backend</h3>
<p>
  <img src="https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white" />
  <img src="https://img.shields.io/badge/Livewire-FB70A9?style=flat-square&logo=livewire&logoColor=white" />
  <img src="https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/C%23-68217A?style=flat-square&logo=csharp&logoColor=white" />
  <img src="https://img.shields.io/badge/.NET-512BD4?style=flat-square&logo=dotnet&logoColor=white" />
</p>

<h3>🎨 Frontend</h3>
<p>
  <img src="https://img.shields.io/badge/Vue.js-35495E?style=flat-square&logo=vuedotjs&logoColor=4FC08D" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/TailwindCSS-38B2AC?style=flat-square&logo=tailwindcss&logoColor=white" />
  <img src="https://img.shields.io/badge/Bootstrap-563D7C?style=flat-square&logo=bootstrap&logoColor=white" />
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" />
</p>

<h3>🗄️ Database & Tools</h3>
<p>
  <img src="https://img.shields.io/badge/MySQL-00758F?style=flat-square&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black" />
  <img src="https://img.shields.io/badge/Nginx-009639?style=flat-square&logo=nginx&logoColor=white" />
  <img src="https://img.shields.io/badge/Vite-646CFF?style=flat-square&logo=vite&logoColor=white" />
  <img src="https://img.shields.io/badge/REST_API-02569B?style=flat-square" />
</p>

<hr>

<h2 align="center">🚀 Main Skills</h2>

<table align="center">
  <tr>
    <td width="50%" valign="top">
      <h3>⚙️ System Development</h3>
      <ul>
        <li>✔️ Laravel Applications</li>
        <li>✔️ ERP Systems</li>
        <li>✔️ Financial Dashboards</li>
        <li>✔️ REST APIs</li>
        <li>✔️ Business Rule Automation</li>
      </ul>
    </td>
    <td width="50%" valign="top">
      <h3>🔧 Technical Experience</h3>
      <ul>
        <li>✔️ MySQL Database Modeling</li>
        <li>✔️ Livewire Components</li>
        <li>✔️ Vue Interfaces</li>
        <li>✔️ Linux Servers</li>
        <li>✔️ Nginx Deployment</li>
      </ul>
    </td>
  </tr>
</table>

<hr>

<h2 align="center">🌍 Connect With Me</h2>

<p align="center">
  <a href="SEU_LINKEDIN">
    <img src="https://img.shields.io/badge/LinkedIn-Igor%20Frederico-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:SEU_EMAIL">
    <img src="https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>
